...
seznam_vzoru=[]
with open("csv/"+subarea+'.csv', 'r',  encoding='utf-8') as csvfile:
...
with open("csv/problem_img.csv", 'r', encoding='utf-8') as csvfile:
    spamreader = csv.DictReader(csvfile, delimiter=',', quotechar='"')
    for row in spamreader:
...
database_easy=[]
with open("easy.csv", 'r', encoding='utf-8') as csvfile:
    spamreader = csv.DictReader(csvfile, delimiter=',', quotechar='"')
    for row in spamreader:
        database_easy.append(row['title'])

database_clonable=[]
with open("clonable.csv", 'r', encoding='utf-8') as csvfile:
    spamreader = csv.DictReader(csvfile, delimiter=',', quotechar='"')
    for row in spamreader:
...
for row in database:
    ...
    testenmd5=hashlib.md5(testen.encode('utf-8')).hexdigest()
    testcsmd5=hashlib.md5(testcs.encode('utf-8')).hexdigest()
    testplmd5=hashlib.md5(testpl.encode('utf-8')).hexdigest()
    testskmd5=hashlib.md5(testsk.encode('utf-8')).hexdigest()
    testesmd5=hashlib.md5(testes.encode('utf-8')).hexdigest()    
...
#        row['question_pl']="NOT TRANSLATED"
        ...
        for hledamklon in database:
            if hledamklon['source_nid']==row['nid']:
                for lang2 in langlist:
                    ...
                    if lang2=="en":
                        linklang2=""
                    outstr=outstr+"<div class='container lang"+lang2+"'><a href=\"https://math4u.vsb.cz/"+linklang2+"problem/"+hledamklon['project_id']+"\" target=\"_blank\"><span class='lang'>"+lang2+"</span></a><div class='question'>"
    ...
    if row['project_id']+"_en" not in databasemd5 or databasemd5[row['project_id']+"_en"][0]!=testenmd5:
    ...
    if row['project_id']+"_cs" not in databasemd5  or databasemd5[row['project_id']+"_cs"][0]!=testcsmd5:
    ...
    if row['project_id']+"_sk" not in databasemd5  or databasemd5[row['project_id']+"_sk"][0]!=testskmd5:
    ...
    if row['project_id']+"_pl" not in databasemd5  or databasemd5[row['project_id']+"_pl"][0]!=testplmd5:
    ...
    if row['project_id']+"_es" not in databasemd5  or databasemd5[row['project_id']+"_es"][0]!=testesmd5:
        if testesmd5 != testenmd5 and testesmd5 != testcsmd5:
            databasemd5[str(row['project_id'])+"_es"]=[testesmd5,str(datetime.now()).split(" ")[0]]
            with open("html/"+row['subarea_id']+"/"+row['project_id']+"_es.html", 'w') as the_file:
                the_file.write(outstr)
